...
void	GetCursorPos (POINT *pos)
{
	...
	else
	{
		...
		if (rect.left == rect.right)
			pos->x = 0;
        else	pos->x = pos->x * Config::SCREEN_SIZE_X / ( rect.right - rect.left );
		if (rect.top == rect.bottom)
			pos->y = 0;
        else	pos->y = pos->y * Config::SCREEN_SIZE_Y / ( rect.bottom - rect.top );
	}
...
void	SetCursorPos (int x, int y)
{
	...
	else
	{
		RECT rect;
		GetClientRect(hMainWnd, &rect);
        pos.x = pos.x * ( rect.right - rect.left ) / Config::SCREEN_SIZE_X;
        pos.y = pos.y * ( rect.bottom - rect.top ) / Config::SCREEN_SIZE_Y;
...
void	GenerateNTSC (int hue, int sat)
{
    ...
    /*
    // TEMP: Save the palette to file. 9-bit index, top 3 bits are emphasis, bottom 6 bits are the color index.
    FILE* fp = fopen( "raw-palette.bin", "wb" );
    for ( int emp = 0; emp < 8; ++emp )
    {
        for ( int color = 0; color < 64; ++color )
        {
            ...
            {
                fputc( RawPalette[ emp ][ color ][ comp ], fp );
            }
