...
// Hack to get immediate operand to correctly register as "executed" and not
// as "read" in CDL.
bool immediateAm;
...
unsigned char MemGetCommon( unsigned int Addr, bool isDummy )
{
	int buf;
    ...
    // If we didn't get any data from other handlers, try to get some from
    // DebugExt.
    if ( buf == -1 )
        buf = DebugExt::readMemory( Addr );

	if (buf != -1)
		LastRead = (unsigned char)buf;

#ifndef NSFPLAYER
    if(Addr == DebugExt::NSF_JSR_INIT || Addr == DebugExt::NSF_JSR_INIT+3 ||
        Addr == DebugExt::NSF_JSR_PLAY || Addr == DebugExt::NSF_JSR_PLAY+3)
    {
        DebugExt::HandleNSFRoutineJump(Addr);
    }
#endif
...
unsigned char MemGetCode( unsigned int Addr, bool isDummy = false )
{
    DebugExt::MemoryCodeReadEvent( Addr, isDummy );

    return MemGetCommon( Addr, isDummy );
}

unsigned char	MemGet( unsigned int Addr, bool isDummy )
{
    if ( immediateAm )
    {
        immediateAm = false;
        return MemGetCode( Addr, isDummy );
    }

    DebugExt::MemoryReadEvent( Addr, isDummy );
...
#ifndef NSFPLAYER
    if(Addr >= 0x401E && Addr <= 0x403F)
    {
        DebugExt::HandleTimer(Addr);
    }
    else if( Addr == 0x4040 )
    {
        DebugExt::HandleDebugOutput();
    }
    else if( Addr >= 0x4041 && Addr <= 0x4042 )
    {
        DebugExt::HandleProfilingStartEnd( Addr );
    }
    else if( Addr == 0x4043 )
    {
        DebugExt::HandleDebuggerBreak( CPU::OpAddr );
    }
    DebugExt::MemoryWriteEvent( Addr, Val );

    DebugExt::writeMemory( Addr, Val );
#endif
}
...
__forceinline	void	AM_IMM (void)
{
    ...
    // Hack, log as a CodeRead event, because it wouldn't otherwise get logged
    // as an instruction byte because CalcAddr is used to fetch the byte.
    // \todo Somehow get rid of the normal event...
    //DebugExt::MemoryCodeReadEvent( CalcAddr, false );
}
...
#ifndef NSFPLAYER
    // If absolute addressing is used with hibyte as zero, zeropage
    // addressing could've been used instead (unless the instruction is
    // JMP)
    if( CalcAddrH == 0 && Opcode != 0x4C && Opcode != 0x6C )
    {
        DebugExt::AbsAddrDiagnostic( CalcAddrL );
    }
#endif
}
