...
unsigned short	DrawArray[Config::SCREEN_SIZE_X*Config::SCREEN_SIZE_Y];
...
void	Reset (void)
{
    ...
    // Align the gfx data pointer with SLnum/Clockticks (can't render the
    // left border, because it's supposed to be at the end of scanline -1).
    GfxData += Config::LEFT_BORDER_SIZE + 1;
}
...
__inline void	RunNoSkip (int NumTicks)
{
	...
	for (i = 0; i < NumTicks; i++)
	{
		...
		else if (Clockticks == EndSLTicks)
		{
			...
			{
				IsRendering = OnScreen = FALSE;
				GetGFXPtr(); // Resets the draw buffer pointer.
                DebugExt::triggerLuaHook();
				GFX::DrawScreen();
			}
        ...
        if ( ( !IsRendering ) && ( ( VRAMAddr & 0x3F00 ) == 0x3F00 ) )
            borderPalIndex = Palette[VRAMAddr & 0x1F];
        // \todo Not sure if grayscale affects PAL NES border color, also not
        //       sure if it's black regardless of where VRAM address points.
        if ( IsPAL )
            borderPalIndex = 0xF;
        borderPalIndex &= GrayScale;
        borderPalIndex |= ColorEmphasis;

        // Left border on scanlines 0..238 and on the one preceding the
        // first real scanline.
        if ( SLnum >= -1 && SLnum <= 238 )
        {
            if ( Clockticks >= 341 - Config::LEFT_BORDER_SIZE )
            {
                //*GfxData++ = 10; // DEBUG
                // On the leftmost border pixel, apply grayscale always ("pulse").
                if ( Clockticks == 341 - Config::LEFT_BORDER_SIZE )
                    *GfxData++ = borderPalIndex & 0x30;
                else
                    *GfxData++ = borderPalIndex;
            }
        }

        // If short scanline, need to add the missing pixel so the screen
        // doesn't shake. Hack! :)
        if ( SLnum == -1 && Clockticks == EndSLTicks - 1 && EndSLTicks == 340 )
            //*GfxData++ = 10; // DEBUG
            *GfxData++ = borderPalIndex;

        // Right border on scanlines 0..239
        if ( SLnum >= 0 && SLnum <= 239 )
        {
            if ( Clockticks >= 256 && Clockticks < 256 + Config::RIGHT_BORDER_SIZE )
            {
                //*GfxData++ = 20; // DEBUG
                *GfxData++ = borderPalIndex;
            }
...
void	__fastcall	Write7 (int Val)
{
	...
	if (IsRendering)
	{
        ...
        DebugExt::writeTo2007WhenRendering();
	}
	else
	{
		if (Reg2000 & 0x04)
